...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && conf.FULLEVA !== 'true') {        
        ...
        await axios.get('http://api.brainshop.ai/get?bid=159379&key=WoOzPYefwUkO0Jam&uid=' + unique_ident + '&msg=' + uren).then(async (response) => {
            var fins = ''                           
            if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                await axios.get('http://api.brainshop.ai/get?bid=159379&key=WoOzPYefwUkO0Jam&uid=' + unique_ident + '&msg=' + son).then(async (response) => {
                    var trmsg = ''
